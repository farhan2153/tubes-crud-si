...
@section('content')
    <div class="d-flex justify-content-center flex-column align-items-center">
        <h4>Edit Maskapai Pesawat</h4>
        <form action="/maskapai/update/{{$maskapai->id}}" method="POST" class="w-25 mt-3">
            @csrf
            @method('PUT')
            <div class="form-group">
                <label for="nama_pesawat" class="form-label">Nama Maskapai Pesawat</label>
                <input type="text" class="form-control" id="nama_pesawat" name="nama_pesawat" placeholder="Masukkan Nama Maskapai Pesawat" value="{{$maskapai->nama_pesawat}}">
            </div>
            <div class="form-group">
